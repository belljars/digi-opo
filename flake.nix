        ...
        in
        {
          default = pkgs.mkShell {
            ...
            shellHook = ''
              ...
              echo "digi-opo Nix-shell valmis. Kaynnista: ./scripts/linux/run_linux.sh"
            '';
          };
        });
      ...
      apps = forAllSystems (system:
        let
          ...
          launcher = pkgs.writeShellScript "digi-opo-launcher" ''
            if [ ! -x ./scripts/linux/run_linux.sh ]; then
              echo "Suorita nix run repojuuresta, jossa ./scripts/linux/run_linux.sh on olemassa." >&2
              exit 1
            fi
            exec ./scripts/linux/run_linux.sh "$@"
          '';
        in
        {
          default = {
